document.addEventListener('DOMContentLoaded', () => {
    ...
    function buildLayout() {
        let html = `<section id="dashboard" class="content-section active">${buildDashboard()}</section>`;
    ...
    function handleTabClick(e) {
        ...
        if (tabEl.classList.contains('locked')) {
            showToast('Este treino ainda não está liberado! Foco na fase atual.', 'error');
            return;
        }
    ...
    function handleMainClick(e) {
        ...
        if (target.closest('.calendar-day.has-workout')) {
            editLogKey = target.closest('.calendar-day.has-workout').dataset.logKey;
    ...
    function handleModalAndDifficultyClicks(e) {
        ...
        if (difficultyBtn) {
            ...
            if (activeModal.id === 'completion-modal') {
                ...
                const scoreEl = document.getElementById('modal-score'),
                    placeholderEl = document.getElementById('modal-score-placeholder');
                if (scoreEl && placeholderEl) {
        ...
        if (activeModal.id === 'completion-modal' && e.target.closest('#save-workout-btn')) {
            ...
            if (!selectedDiff) {
                showToast('Selecione um nível de dificuldade.', 'error');
                return;
            }
            const todayKey = getTodayKey(),
                log = workoutLog[todayKey];
            ...
            if (currentCard) {
                currentCard.querySelectorAll('.exercise-checkbox').forEach(cb => cb.checked = false);
                updateProgressBar(currentCard);
                checkFinishButtonState(currentCard);
            }
    ...
    function updateProgressBar(workoutCard) {
        const progressBarContainer = workoutCard.querySelector('.progress-bar-container');
        ...
        const checked = workoutCard.querySelectorAll('.exercise-list .exercise-checkbox:checked');
        const progress = checkboxes.length > 0 ? (checked.length / checkboxes.length) * 100 : 0;
        progressBarFill.style.width = `${progress}%`;
        progressBarContainer.style.opacity = progress > 0 ? '1' : '0';
    }

    function checkFinishButtonState(workoutCard) {
        const finishBtn = workoutCard.querySelector('.finish-workout-btn');
        const checkedCount = workoutCard.querySelectorAll('.exercise-list .exercise-checkbox:checked').length;
        if (finishBtn) finishBtn.disabled = checkedCount === 0;
    }
    ...
    function savePlanChanges(workoutCard) {
        ...
        workoutCard.querySelectorAll('.exercise-list-edit li').forEach(li => {
            ...
            if (nameInput.value && repsInput.value) {
                newExercises.push({
                    id,
                    text: `<strong>${nameInput.value.trim()}:</strong> ${repsInput.value.trim()}`
                });
            }
        });
    ...
    function addDay(button) {
        ...
        if ((phase.days || []).length >= 7) {
            showToast("Limite de 7 dias por fase atingido.", "error");
            return;
        }
        const newDayNumber = (phase.days || []).length + 1;
        const newDay = {
            idPrefix: `${phaseKey.toLowerCase()}-d${newDayNumber}`,
            title: `Dia ${newDayNumber}: Novo Treino`,
            exercises: [{
                id: `ex-${Date.now()}`,
                text: "<strong>Novo Exercício:</strong> 3 séries de 10 reps"
            }]
        };
    ...
    function calculateSmartScore(log) {
        ...
        else if (diff === 2) difficultyDelta = -0.5;
        else if (diff === 4) difficultyDelta = 0.75;
        else if (diff === 5) difficultyDelta = 1.25;
        let cardioDelta = 0;
        if (log.cardio && log.cardio.minutes > 0) {
            if (log.cardio.minutes < 20) cardioDelta = 0.5;
            else cardioDelta = 1.5;
        }
    ...
    function finishWorkout(workoutCard) {
        ...
        if (workoutLog[todayKey]) {
            showConfirm("Sobrescrever Treino?", "Você já registrou um treino hoje. Deseja sobrescrevê-lo?", () => proceedWithFinish(workoutCard));
        } else {
            proceedWithFinish(workoutCard);
        }
    }
    ...
    function proceedWithFinish(workoutCard) {
        ...
        if (cardioMinutes > 0) {
            ...
            log.cardio = {
                ...
                difficulty: cardioDiffBtn ? parseInt(cardioDiffBtn.dataset.value, 10) : 3
            };
        } else {
            delete log.cardio;
        }
        ...
        const scoreEl = modal.querySelector('#modal-score'),
            placeholderEl = modal.querySelector('#modal-score-placeholder');
    ...
    function populateEditModal(logKey) {
        ...
        const phase = userWorkoutPlan[log.phase];
        ...
        if (day) {
            ...
            exContainer.innerHTML = '<p class="text-gray-500">Não foi possível carregar a lista de exercícios (o plano pode ter sido alterado).</p>';
        }
    ...
    function saveEdits() {
        ...
        if (!workoutDiffBtn) {
            showToast('Selecione uma dificuldade para o treino.', 'error');
            return;
        }
        ...
        if (cardioMins > 0) {
            ...
            log.cardio = {
                type: document.getElementById('edit-cardio-type')?.value,
                minutes: cardioMins,
                difficulty: cardioDiffBtn ? parseInt(cardioDiffBtn.dataset.value, 10) : null
            };
        } else {
            delete log.cardio;
        }
    ...
    function renderDashboard() {
        const today = new Date(),
            currentMonthStr = today.toISOString().slice(0, 7),
            logs = Object.entries(workoutLog);
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (workoutLog[dayKey]) {
                ...
                dayDiv.addEventListener('mouseover', (e) => {
                    const log = workoutLog[dayKey];
                    const tooltip = document.getElementById('calendar-tooltip');
                    tooltip.innerHTML = `${log.dayTitle}<br>Nota: <strong>${log.smartScore.toFixed(1)}</strong>`;
    ...
    function init() {
        try {
            ...
            if (!userWorkoutPlan.A || !userWorkoutPlan.B || !userWorkoutPlan.C) throw new Error("Plano inválido");
        } catch (e) {
            console.error("Falha ao carregar o plano, redefinindo para o padrão.", e);
